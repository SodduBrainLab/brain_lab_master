...
def monte_carlo_metropolis(J, spin_vec, t, iterations, thermalize_time=None, phi_variables=False, type='digital',
                           return_tc=False,local_temp=False, pet_suv=[],alpha=-1):
    ...
    if not local_temp:
        ...
        assert np.array(pet_suv).shape[0] == J.shape[0]
        ...
        for i in range(int(iterations / no_spin)):
            ...
            for j in range(no_spin):
                ...
                if d_e <= 0 or random() <= np.exp(-d_e / t*(pet_suv[j]**alpha)):
                    spin_vec[spin_permutation[j]] = -1 * new_spin
...
def compute_par(values):
    ...
    for tT in range(values[1], values[2]):
        ...
        if phi_variables:
            es, ess, ms, mss, spin_bin_sum = monte_carlo_metropolis(values[0], spin_vec, ts[tT], no_flip, values[4],
                                                                    phi_variables=phi_variables, type=type,local_temp=local_temp,pet_suv=pet_suv,alpha=alpha)
        ...
        for sim in range(values[3]):
            simulation[:, sim] = monte_carlo_metropolis(values[0], spin_vec, ts[tT], n, phi_variables=phi_variables,
                                                        type=type,local_temp=local_temp,pet_suv=pet_suv,alpha=alpha)
    ...
    if return_tc:
        if phi_variables:
            return (np.copy(E), np.copy(M), np.copy(S), np.copy(H), np.copy(simulated_fc), np.copy(values[1]),
                    np.copy(values[2]), np.asarray(spin_mean), np.asarray(tc))
        else:
            return (np.copy(E), np.copy(M), np.copy(S), np.copy(H), np.copy(simulated_fc), np.copy(values[1]),
                    np.copy(values[2]), np.asarray(tc))
    else:
        if phi_variables:
            return (np.copy(E), np.copy(M), np.copy(S), np.copy(H), np.copy(simulated_fc), np.copy(values[1]),
                    np.copy(values[2]), np.asarray(spin_mean))
        else:
            return (np.copy(E), np.copy(M), np.copy(S), np.copy(H), np.copy(simulated_fc), np.copy(values[1]),
                    np.copy(values[2]))


def generalized_ising(Jij, temperature_parameters=(0.1, 5, 100), n_time_points=100, thermalize_time=0.3,
                      temperature_distribution='lineal', phi_variables=False, return_tc=False, type='digital',local_temp = False,pet_suv=[],alpha=-1):
    n = Jij.shape[-1]
    ...
    for next in range(n_cpu):
        if (next + 1) * step_len > temperature_parameters[2]:
            l.append((Jij, previus, temperature_parameters[2], n_time_points, thermalize_time, ts, phi_variables, type,
                      return_tc,local_temp,pet_suv,alpha))
        else:
    ...
    if return_tc:
        ...
        if phi_variables:
            ...
            return np.copy(simulated_fc), np.copy(critical_temperature), np.copy(E), np.copy(M), np.copy(S), np.copy(
                H), np.copy(spin_mean), np.copy(tc)
        else:
            for i in range(results.shape[0]):
                E[results[i, 5]:results[i, 6]] = results[i, 0]
                M[results[i, 5]:results[i, 6]] = results[i, 1]
