...
def percent_diff(df,n,states,network):
    ...
    for state in states:
        ...
        d_corr = np.array(taken_set.correlation) - np.array(rest_set.correlation)
        norm = (np.array(taken_set.correlation) + np.array(rest_set.correlation)) / 2

        percent_diff = 100 * d_corr / norm
...
df = pd.read_csv(main_path+'tot_corr.csv')
...
plt.savefig(main_path+'fig_diff_net_corr.pdf')
