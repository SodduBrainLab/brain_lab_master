...
df.rename(columns={'correlation':'correlation','task':'task','state':'Sedation Level','network':'network'},inplace=True)
...
titles = [r'$\langle |\rho| \rangle$ modulated by Sedation Level (Rest)',r'$\langle |\rho| \rangle$ modulated by Sedation Level (Taken)']
for ind, axes in enumerate(ax.axes.flat):
    axes.set_title(titles[ind],usetex=True,fontsize=14)
    axes.set_ylabel(r'$\langle |\rho| \rangle$  ', usetex=True, fontsize=20, rotation=0)
...
plt.savefig(main_path+'fig_corr_abs.pdf')
